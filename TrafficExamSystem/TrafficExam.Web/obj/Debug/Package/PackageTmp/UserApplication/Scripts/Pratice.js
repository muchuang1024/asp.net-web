...
function SubmitAnswer() 
{
    ...
    if ($("#lblResult").text() == "") {
        ...
        $.ajax({ global: true, url: "PraticePaperIndexRequest.aspx", data: { ActionName: "UpdateUserAnswer", TitleId: titleId, UserAnswer: userChooseAnswer, IsProper: isProper },
...
function SetPaperList() {
    ...
    $.ajax({ global: true, url: "PraticePaperIndexRequest.aspx", data: { ActionName: "SetPaperList", TableName: tableName },
        success: function (data) {
            if (data == true) {
                pageIndex = 1;
...
function LoadPaperInfo(pageIndex) {
    //ajax验证数据
    $.ajax({ global: true, url: "PraticePaperIndexRequest.aspx", data: { ActionName: "GetPaperInfo", TableName: tableName,TitleId: pageIndex },
        success: function (data) {
            if (data != null) {
                ShowPaperInfo(data);
            }
...
function GetIsProperList() {
    //ajax验证数据
    $.ajax({ global: true, url: "PraticePaperIndexRequest.aspx", data: { ActionName: "GetIsProperList" },
        success: function (data) {
            if (data != null) {
                ShowExamCard(data);
            }
...
function SubmitExam() {
    if (confirm("您是否确认提交试卷！") == true)//这里要特别注意"!"感叹号的全区/半区区分
    {
        ...
        //ajax验证数据
        $.ajax({ global: true, url: "PraticePaperIndexRequest.aspx", data: { ActionName: "SubmitExam", TableName: tableName },
            success: function (data) {
                if (data >= 0) {
                    alert("本次考试成绩：" + data);
